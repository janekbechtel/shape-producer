...
def get_triggerweight_for_channel(channel):
    ...
    if "mt" in channel:
        ...
        MuTauMC = "*".join([trig_sL, singleMC]) + "+" + "*".join([trig_X, crossMC]) # TODO add TauCrossTrigger Weight back in
...
def get_eleRecoWeight_for_channel(channel):
    ...
    if "em" in channel:
        weight = Weight("(eleRecoWeight_1)","eleRecoWeight") # TODO all this in next ntuples
    return weight
...
class ZTTEmbeddedEstimation(EstimationMethod):
    ...
    def get_weights(self):
        ...
        if self.channel.name == "mt":
            ...
            emb_weights.add(Weight("(gen_match_1==3 && gen_match_2==4)", "emb_veto")) #TODO add trigger sf as soon as they are included
            emb_weights.add(Weight("idWeight_1*isoWeight_1*idWeight_2*isoWeight_2",
                       "leptopn_sf"))
            emb_weights.remove("decayMode_SF") # embeddedDecayModeWeight is only for tau decay modes
        return emb_weights
...
# class VVEstimation(EstimationMethod): # TODO swap back to newer Estimation with Di-Boson Samples as soon as samples are available
#     def __init__(self, era, directory, channel, friend_directory=None, folder="nominal",
#             get_triggerweight_for_channel=get_triggerweight_for_channel,
#             get_singlelepton_triggerweight_for_channel=get_singlelepton_triggerweight_for_channel,
#             get_tauByIsoIdWeight_for_channel=get_tauByIsoIdWeight_for_channel,
