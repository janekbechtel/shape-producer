...
def get_tauByIsoIdWeight_for_channel(channel):
    ...
    if "mt" in channel or "et" in channel:
        ...
        weight = Weight("((gen_match_1 == 5)*0.87 + (gen_match_1 != 5))*((gen_match_2 == 5)*0.87 + (gen_match_2 != 5))", "taubyIsoIdWeight")
    return weight
...
class HTTEstimation(EstimationMethod):
    ...
    def get_weights(self):
        if self.channel.name=="em":
            ...
            return Weights(
                Weight("identificationWeight_1*identificationWeight_2","identificationWeight"),
                #Weight("idIsoWeight_1*idIsoWeight_2","idIsoWeight"),
                Weight("triggerWeight_1*triggerWeight_2","triggerWeight_dy"),
...
class ZTTEmbeddedEstimation(EstimationMethod):
    ...
    def embedding_tauid(self):
        if self.channel.name == "et" or self.channel.name == "mt":
            return Weight("(gen_match_2==5)*0.87+(gen_match_2!=5)", "emb_tau_id") # TODO measure for now MC correction factors
        elif self.channel.name == "tt":
            return Weight("((gen_match_1==5)*0.87+(gen_match_1!=5))*((gen_match_2==5)*0.870+(gen_match_2!=5))", "emb_tau_id"),
        else:
    ...
    def get_weights(self):
        emb_weights = Weights(
            ...
            Weight("muonEffTrgWeight", "scale_factor"), # TODO put back in with new ntuples *muonEffIDWeight_1*muonEffIDWeight_2
        ...
        if self.channel.name == "mt":
            ...
            emb_weights.add(Weight("gen_match_1==3 && gen_match_2==5","emb_veto"))
    ...
    def get_weights(self):
        if self.channel.name=="em":
            ...
            return Weights(
                ...
                Weight("prefiringweight", "prefireWeight"),
                Weight("generatorWeight", "generatorWeight"),
                Weight("(5.190747826298e-6)/(numberGeneratedEventsWeight*crossSectionPerEventWeight)",
                    "EWKWp_stitching_weight"),
                self.era.lumi_weight)
        ...
        if self.channel.name=="em":
            return Weights(
             ...
             return Weights(
                ...
                Weight("numberGeneratedEventsWeight","numberGeneratedEventsWeight"),
                Weight("generatorWeight", "generatorWeight"),
                Weight("(4.200367267668e-6)/(numberGeneratedEventsWeight*crossSectionPerEventWeight)",
    ...
    def get_weights(self):
        if self.channel.name=="em":
            ...
            return Weights(
                ...
                # MC weights
                Weight("generatorWeight", "generatorWeight"),
                Weight("(((npartons == 0 || npartons >= 5)*7.09390278348407e-4) + ((npartons == 1)*1.90063898596475e-4) + ((npartons == 2)*5.8529964471165e-5) + ((npartons == 3)*1.9206444928444e-5) + ((npartons == 4)*1.923548021385e-5))/(numberGeneratedEventsWeight*crossSectionPerEventWeight*sampleStitchingWeight)",
                    "wj_stitching_weight"),
                self.era.lumi_weight)
